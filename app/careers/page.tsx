...
const benefitIcons = [Heart, Users, GraduationCap, Globe]

export default function CareersPage() {
  const { t } = useI18n()
  const benefits = t.careers.benefits as { title: string; desc: string }[]
  const jobs = t.careers.jobs as { title: string; type: string; location: string; desc: string }[]
      ...
      <div className="py-24 md:py-32">
        <div className="max-w-[1800px] mx-auto px-6 md:px-12">
          <div className="text-center mb-16 md:mb-24">
            ...
            <h1 className="font-serif text-5xl md:text-7xl tracking-tight mb-6">{t.careers.title}</h1>
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">{t.careers.desc}</p>
          </div>
          ...
          <div className="max-w-4xl mx-auto">
            <h2 className="font-serif text-3xl mb-8 text-center">{t.careers.jobsTitle}</h2>
              ...
              {jobs.map((job, i) => (
                <div key={i} className="p-6 border border-border flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-secondary/10 transition-colors">
                  ...
                  <button className="whitespace-nowrap px-6 py-2 border border-accent text-accent text-sm tracking-wider uppercase hover:bg-accent hover:text-accent-foreground transition-colors flex items-center gap-2">
                    {t.careers.apply} <ArrowRight className="w-3 h-3" />
                  </button>
                </div>
              ))}
            </div>
            <p className="text-center text-sm text-muted-foreground mt-8">{t.careers.spontaneous}</p>
          </div>
        </div>
      </div>
    </div>
